...
def _build_short_pipeline(cfg, state: StateStore, slot: int, date_yyyymmdd: str):
    ...
    seed = _seed_for(slot, date_yyyymmdd)
    quiz, used_seed = _ensure_unique_or_regen(state, cfg, seed)

    date_iso = datetime.strptime(date_yyyymmdd, "%Y%m%d").strftime("%Y-%m-%d")
    fp = state.add_used_question(quiz.question, quiz.answer, date_iso)
    ...
    tts_provider = synthesize_tts(cfg, spoken, tts_wav)
    ...
    hashtags = " ".join([h.strip() for h in quiz.hashtags if h.strip()])
    desc = quiz.description.strip()
    if hashtags:
        desc = (desc + "\n\n" + hashtags).strip()
...
def _build_long_pipeline(cfg, state: StateStore, date_yyyymmdd: str) -> str:
    ...
    title = f"Daily Quiz Compilation ({date_yyyymmdd})"
    desc = "Daily compilation of today's 10-second quizzes.\n\nSubscribe for more!"
    tags = ["quiz", "trivia", "compilation", "daily quiz", "brain teaser", "knowledge", "challenge"]
